...
function LanguageSelector({className}:{className?:string}) {
  ...
  return (
      ...
      <div className="relative w-full" onBlur={() => setOpen(false)}>
        <button onClick={() => setOpen(true)} className={cn("focus:ring-2 border-2 bg-white mb-1 border-primary text-primary p-0.5  px-2 font-semibold rounded-full flex items-center ",className)}><span>{language?.lang}</span><Globe className="ml-1" size={20} /></button>
        {
          open &&
